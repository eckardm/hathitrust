'''
first things first, import the python libraries we'll need, listed in the order you'll need them'''
...
regents_proceedings = 'https://babel.hathitrust.org/cgi/mb?a=listis;c=1010993475;sz=75'

# run it!
get_volume_ocr(regents_proceedings)

'''
get ocr for entire run'''

# go through each file in the ocr folder
for ocr_file in os.listdir('ocr'):
    # open the ocr file
    ocr = open(join('ocr', ocr_file), 'r')
    # go through each line in the ocr file
    for line in list(ocr):
